...
impl Client {
    fn new(addr: SocketAddr, duration_ping: u64) -> Option<Self> {
        ...
        let mut buf = [0; size_of::<TypeSize>() * 2];
        if stream_read.lock().unwrap().read_exact(&mut buf).is_err() {
            return None;
        }

        let v = buf.chunks(size_of::<TypeSize>()).map(|x| TypeSize::from_be_bytes(x.try_into().unwrap())).collect::<Vec<_>>();
        Some(Self { stream_read, stream_write, rank: v[0], workers_num: v[1], })
    }
    pub fn get_rank(&self) -> TypeSize {
        self.rank
    }
    pub fn get_workers_num(&self) -> TypeSize {
        self.workers_num
    }
    pub fn send_to_async(&mut self, dest: TypeSize, data: &[u8]) -> JoinHandle<()> {
    ...
    pub fn send_to(&mut self, dest: TypeSize, data: &[u8]) {
    ...
    pub fn recieve_async(&mut self) -> JoinHandle<(TypeSize, Vec<u8>)> {
        let stream = self.stream_read.clone();
        thread::spawn(move || {
            let mut stream = stream.lock().unwrap();
            let mut buf = vec![0; size_of::<TypeSize>() * 2];
    ...
    pub fn recieve(&mut self) -> (TypeSize, Vec<u8>) {
        let mut buf = vec![0; size_of::<TypeSize>() * 2];
